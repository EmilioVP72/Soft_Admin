...
<body class="p-4">
    ...
    <header class="flex justify-between items-end border-b-2 border-slate-200 pb-6 mb-8">
        <div class="flex items-center gap-4">
            <div class="w-16 h-16 bg-red-700 rounded-lg shadow-sm flex items-center justify-center text-white font-bold text-xl tracking-tighter">
                {{ config('app.name', 'DUO') }}
            </div>
            <div>
                <h1 class="text-2xl font-bold text-slate-900 leading-tight">Reporte de Salidas</h1>
                <p class="text-sm text-slate-500 font-medium mt-0.5">Movimientos Registrados</p>
            </div>
        </div>
        <div class="text-right">
            <h2 class="text-3xl font-black text-red-700 uppercase tracking-widest mb-1">SALIDAS</h2>
    ...
    <section class="grid grid-cols-2 gap-6 mb-8 w-full">
        ...
        <div class="bg-red-50 p-5 rounded-xl border border-red-100 shadow-sm relative">
            <p class="text-sm text-red-600 font-semibold mb-1 uppercase tracking-wide">Total Monto Salida</p>
            <p class="text-2xl font-bold text-red-800 break-words leading-tight">${{ number_format($monto_total, 2) }}</p>
        </div>
    </section>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($movimientos as $mov)
                        <tr class="hover:bg-slate-50 transition-colors {{ $loop->even ? 'bg-slate-50/50' : '' }}">
                            <td class="px-5 py-3.5 font-bold text-red-700">#{{ $mov->id_transaction }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-8 text-center text-slate-500 font-medium">No se encontraron salidas registradas en este período.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
