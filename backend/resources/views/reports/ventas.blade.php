...
<head>
    ...
    <style>
        ...
        @page {
            size: A4 portrait;
            margin: 0; /* Quitamos margen de la página para controlarlo con padding en el body */
        }
        body {
        ...
        @media print {
            body {
                width: 210mm; /* Forzar el ancho de la página A4 */
                padding: 10mm 15mm; /* Margen interno para dar espacio a los bordes */
                margin: 0;
            }
            ...
            tfoot { display: table-row-group; } /* Evita que el footer se repita en cada hoja */
            .grid, .flex { page-break-inside: avoid; }
        }
    </style>
</head>
<body class="p-8 w-full max-w-[210mm] mx-auto">
    ...
    <section class="grid grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 p-3 opacity-10 text-blue-700">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
                </svg>
            </div>
            <p class="text-xs text-slate-500 font-semibold mb-1 uppercase tracking-wide truncate">Operaciones</p>
        ...
        <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <p class="text-xs text-slate-500 font-semibold mb-1 uppercase tracking-wide truncate">Ticket Promedio</p>
            <p class="text-2xl font-bold text-slate-900 truncate">${{ number_format($ticket_promedio, 2) }}</p>
        </div>
    </section>

    <main>
        <div class="rounded-xl border border-slate-200 shadow-sm relative w-full overflow-hidden">
            <table class="w-full text-left text-xs table-fixed">
                ...
                <thead class="bg-slate-100 text-slate-600 border-b border-slate-200">
                    <tr>
                        <th scope="col" class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Fecha / Hora</th>
                        <th scope="col" class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">ID Venta</th>
                        <th scope="col" class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Sucursal</th>
                        <th scope="col" class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Pago</th>
                        <th scope="col" class="px-4 py-3 font-bold uppercase tracking-wider text-[10px] text-right">Monto</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($ventas as $venta)
                        <tr class="hover:bg-slate-50 transition-colors {{ $loop->even ? 'bg-slate-50/50' : '' }}">
                            <td class="px-4 py-3 text-slate-500 whitespace-nowrap">{{ \Carbon\Carbon::parse($venta->transaction_date)->format('d M y, H:i') }}</td>
                            <td class="px-4 py-3 font-semibold text-blue-700 whitespace-nowrap">#{{ $venta->id_transaction }}</td>
                            <td class="px-4 py-3 font-medium text-slate-700 truncate" title="{{ $venta->store->store ?? 'N/A' }}">{{ $venta->store->store ?? 'N/A' }}</td>
                            <td class="px-4 py-3">
                                @php
                            ...
                            <td class="px-4 py-3 text-right font-bold text-slate-800 whitespace-nowrap">${{ number_format($venta->total_amount, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tfoot>
                    <tr class="bg-slate-800 text-white">
                        <td colspan="4" class="px-4 py-3 text-right font-bold tracking-wide">TOTAL DEL REPORTE:</td>
                        <td class="px-4 py-3 text-right font-black text-sm whitespace-nowrap">${{ number_format($total_recaudado, 2) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
