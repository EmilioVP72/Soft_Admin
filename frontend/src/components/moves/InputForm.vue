...
const validateForm = () => {
    ...
    if (formData.value.details.length === 0) {
        showWarning('Sin detalles', 'Debes agregar al menos un detalle para la entrada.');
        return false;
    }
...
const submitForm = async () => {
    ...
    try {
        // Build payload matching Backend validation StoreInputRequest
        const payload = {
        ...
        await MovesServices.createInput(payload);
        emit('saved');
        closeModal();
    } catch (error) {
        showError('Error al guardar', 'Ocurrió un error guardando la entrada. Verifica los datos o intenta más tarde.');
    } finally {
        ...
        <div class="modal-content">
            ...
            <form @submit.prevent="submitForm" class="modal-body">
                ...
                <div class="form-group full-width">
                    <label>Notas</label>
                        <textarea v-model="formData.notes" class="form-control" rows="2" placeholder="Notas de la entrada..." maxlength="500"></textarea>
                </div>
                ...
                <div class="details-container">
                    <div v-for="(detail, index) in formData.details" :key="index" class="detail-row">
                        ...
                        <div class="form-group flex-1">
                            <label>Cant.</label>
                            <input type="number" step="1" min="1" v-model="detail.quantity" required @input="updateSubtotal(index)" class="form-control form-quantity">
                        </div>
                        <div class="form-group flex-1">
                            <label>Precio Unit.</label>
                            <input type="number" step="0.01" min="0.01" v-model="detail.unit_price" required @input="updateSubtotal(index)" class="form-control form-unit-price">
                        </div>
                        <div class="form-group flex-1">
                            <label>Subtotal</label>
                            <input type="number" v-model="detail.subtotal" readonly class="form-control readonly form-subtotal">
                        </div>
